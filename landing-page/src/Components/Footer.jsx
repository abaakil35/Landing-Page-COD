...
const Footer = () => (
		<footer className="w-full bg-[#5e255dff] text-gray-300 pt-16 pb-8 px-8">
			...
			<div className="md:w-1/4">
				<div className="flex items-center mb-4">
					<div className="w-[100px] h-[32px]rounded-md flex items-center justify-center mr-4 overflow-hidden">
						<img src={logo} alt="COD Rocket Logo" className="h-6 object-contain" />
					</div>
				</div>
				   <p className="mb-6 text-base">The ultimate cash on delivery solution for Shopify stores. Streamline your COD process and boost sales.</p>
				<div className="flex gap-4">
					{socialLinks.map((link, idx) => (
						<a key={idx} href={link.href} target="_blank" rel="noopener noreferrer" aria-label={link.label} className="bg-[#232b36] hover:bg-[#1a202c] p-2 rounded-full transition-colors">
							{link.svg}
						</a>
					))}
				</div>
		...
		<div className="text-center text-gray-400 text-sm">© 2024 COD Rocket. All rights reserved.</div>
	</footer>
);
